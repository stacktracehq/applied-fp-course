...
import           Level06.Types (mkTopic, mkCommentText, getTopic, getCommentText, Error(..))
import qualified Waargonaut.Decode                  as D
import Level06.Conf.File (parseFunc)
import Level06.Types (Port(..), DBFilePath(..), PartialConf(..), partialConfDecoder)
...
decodingConfigTests :: TestTree
decodingConfigTests = testGroup "decoding conf tests"
  [
    let
      json = "{ \"port\": 3000, \"dbName\": \"test.db\" }"
      expectedPort = Port 3000
      expectedDBFilePath = DBFilePath "test.db"
      expected = PartialConf (pure expectedPort) (pure expectedDBFilePath)
    in
      testCase "when both port and dbName present in json" $
        D.simpleDecode partialConfDecoder parseFunc json @?= Right expected
  ,
    let
      json = "{ \"dbName\": \"test.db\" }"
      expectedDBFilePath = DBFilePath "test.db"
      expected = PartialConf (Last Nothing) (pure expectedDBFilePath)
    in
      testCase "when port missing in json" $
        D.simpleDecode partialConfDecoder parseFunc json @?= Right expected
  ,
    let
      json = "{ \"port\": 3000 }"
      expectedPort = Port 3000
      expected = PartialConf (pure expectedPort) (Last Nothing)
    in
      testCase "when dbName is missing in json" $
        D.simpleDecode partialConfDecoder parseFunc json @?= Right expected

  ]

main :: IO ()
main = defaultMain $ testGroup "Applied FP Course - Level 06 Tests" [
    decodingConfigTests
  ]
